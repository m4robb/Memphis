...
namespace WaveHarmonic.Crest
{
    ...
    abstract class QueryBase
    {
        ...
        protected abstract ComputeShader QueriesCompute { get; }
        protected ComputeShader ShaderProcessQueries { get; }

        readonly PropertyWrapperComputeStandalone _Wrapper;
        ...
        public QueryBase(WaterRenderer water)
        {
            ...
            if (_MaximumQueryCount != water._AnimatedWavesLod.MaxQueryCount)
            {
                _MaximumQueryCount = water._AnimatedWavesLod.MaxQueryCount;
            ...
            ShaderProcessQueries = QueriesCompute;
            if (ShaderProcessQueries == null)
            {
                Debug.LogError($"Crest: Could not load Query compute shader");
                return;
            }
            _KernelHandle = ShaderProcessQueries.FindKernel(QueryKernelName);
            _Wrapper = new(ShaderProcessQueries, _KernelHandle);
        }
        ...
        protected bool UpdateQueryPoints(int ownerHash, float minSpatialLength, Vector3[] queryPoints, Vector3[] queryNormals)
        {
            ...
            for (var normi = 0; normi < countNorms; normi++)
            {
                ...
                _QueryPositionXZ_MinimumGridSize[arrIdx + 0].x = queryNormals[normi].x;
                _QueryPositionXZ_MinimumGridSize[arrIdx + 0].y = queryNormals[normi].z;
                _QueryPositionXZ_MinimumGridSize[arrIdx + 0].z = minGridSize;

                _QueryPositionXZ_MinimumGridSize[arrIdx + 1].x = queryNormals[normi].x + k_FiniteDifferenceDx;
                _QueryPositionXZ_MinimumGridSize[arrIdx + 1].y = queryNormals[normi].z;
                _QueryPositionXZ_MinimumGridSize[arrIdx + 1].z = minGridSize;

                _QueryPositionXZ_MinimumGridSize[arrIdx + 2].x = queryNormals[normi].x;
                _QueryPositionXZ_MinimumGridSize[arrIdx + 2].y = queryNormals[normi].z + k_FiniteDifferenceDx;
                _QueryPositionXZ_MinimumGridSize[arrIdx + 2].z = minGridSize;
            }
        ...
        void ExecuteQueries()
        {
            ...
            ShaderProcessQueries.SetBuffer(_KernelHandle, ShaderIDs.s_QueryPositions_MinimumGridSizes, _ComputeBufferQueries);
            BindInputsAndOutputs(_Wrapper, _ComputeBufferResults);

            var numGroups = (_SegmentRegistrarRingBuffer.Current._QueryCount + k_ComputeGroupSize - 1) / k_ComputeGroupSize;
            ShaderProcessQueries.Dispatch(_KernelHandle, numGroups, 1, 1);
        }
        ...
        public int Query(int ownerHash, float minSpatialLength, Vector3[] queryPoints, Vector3[] resultDisplacements, Vector3[] resultNormals, Vector3[] resultVelocities)
        {
        ...
        internal int ResultGuidCount => _ResultSegments != null ? _ResultSegments.Count : 0;

        internal int RequestCount => _Requests != null ? _Requests.Count : 0;
    }
}
