...
namespace WaveHarmonic.Crest.Editor
{
    ...
    sealed class DepthProbeEditor : Inspector
    {
        protected override void RenderBottomButtons()
        {
            ...
            var onDemand = target.Type == DepthProbe.ProbeMode.Realtime && target.RefreshMode == DepthProbe.ProbeRefreshMode.ViaScripting;
            var canBake = !onDemand && !Application.isPlaying;
            var canPopulate = Application.isPlaying ? onDemand : target.Type != DepthProbe.ProbeMode.Baked;

            if (canPopulate && GUILayout.Button("Populate"))
            {
                target.Populate(updateComponents: true);
            }

            if (canBake && GUILayout.Button("Bake"))
            {
                target.ForcePopulate(updateComponents: true);
                ...
                if (target.SavedTexture == null)
                {
                    ...
                    serializedObject.FindProperty(nameof(DepthProbe._Type)).enumValueIndex = (int)DepthProbe.ProbeMode.Baked;
                    serializedObject.ApplyModifiedProperties();
                }
