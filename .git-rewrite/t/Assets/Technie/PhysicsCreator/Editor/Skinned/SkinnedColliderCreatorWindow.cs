...
namespace Technie.PhysicsCreator.Skinned
{
	...
	public class SkinnedColliderCreatorWindow : EditorWindow, ICreatorWindow
	{
		...
		private void DrawBoneHierarchyUi()
		{
			...
			BuildHierarchyList(0, null, targetSkinnedRenderer.rootBone, hierarchyElements);

			int shrink = (hasScrollBar ? 20 : 5);
			hierarchyScrollPosition = GUILayout.BeginScrollView(hierarchyScrollPosition, false, false, GUIStyle.none, GUI.skin.verticalScrollbar, GUILayout.Width(Screen.width-shrink), GUILayout.Height(400));
			...
			for (int i = 0; i < hierarchyElements.Count; i++)
			{
				...
				Rect rowArea = GUILayoutUtility.GetRect(Screen.width - 20, ROW_HEIGHT);
				//Rect rowArea = GUILayoutUtility.GetRect(100, ROW_HEIGHT);
				...
				int indent = (elem.indentLevel+1) * INDENT_AMOUNT;
				...
				if (elem.hasChildren)
				{
					boneFoldoutState.TryGetValue(elem.srcBone, out bool isExpanded);
				...
				if (elem.IsBone)
				{
					//	float xOffset = indent + EXPAND_BUTTON_SIZE + TYPE_ICON_SIZE + textLength;
		...
		{
			BoneData boneData = TargetEditorData.GetBoneData(currentBone.name);
			BoneHullData[] hullData = TargetEditorData.GetBoneHullData(currentBone.name);

			HierarchyElement elem = new HierarchyElement();
			elem.parentElement = parentElement;
			elem.srcBone = currentBone;
			elem.srcBoneData = boneData;
			elem.indentLevel = depth;
			elem.name = currentBone.name;
			elem.hasChildren = currentBone.childCount > 0 || (hullData != null && hullData.Length > 0);
			destElements.Add(elem);

			boneFoldoutState.TryGetValue(currentBone, out bool isExpanded);
...
#pragma warning restore 0162
		...
		private void CreateHullColliders()
		{
			...
			// Finally added the joints connecting the bodies
...
#pragma warning restore 0162
		...
		/*
		private int CalcDepth(Transform bone)
		{
		...
		*/
		/*
		private void UpdateCollider(Transform bone)
		{
			BoneHullData[] hulls = TargetEditorData.GetBoneHullData(bone.name);
			MeshCollider[] cols = bone.GetComponents<MeshCollider>();

			for (int i=0; i<hulls.Length; i++)
			{
				BoneHullData data = hulls[i];
				Mesh hullMesh = CreateHullMesh(TargetRenderer, unpackedMesh, bone, data, CoordSpace.Bone); // TODO: should this update an existing hull mesh if it already exists?

				// TODO: Save this mesh in the asset
				//SkinnedColliderRuntimeData runtimeData = TargetEditorData.runtimeData;
				// ..
				data.hullMesh = hullMesh; // associate this mesh with the hull, will be saved into an asset later

				//Console.output.Log("UpdateCollider for " + bone + " with threshold " + data.threshold);

				// Awkward - we also create MeshColliders here, but that already exists in CreateHullColliders
				MeshCollider col = (i < cols.Length) ? cols[i] : bone.gameObject.AddComponent<MeshCollider>();
				col.convex = true;
				col.sharedMesh = hullMesh;
		...
		*/
		...
		public static Mesh CreateHullMesh(SkinnedMeshRenderer skinnedRenderer, UnpackedMesh unpackedMesh, Transform bone, BoneHullData hull, CoordSpace outputSpace)
		{
			...
			return CalculateConvexHull(bone.name, hullPoints);
		}
		...
		private static Mesh CalculateConvexHull(string meshName, List<Point3d> hullPoints)
		{
			// TODO: There's quite a bit of duplication here between this and the same hull calculation in PaintinData.GenerateConvexHull
			...
			{
				//Console.output.LogError(Console.Technie, "Could not generate hull for " + this.name + "'s '" + hull.name + "' (input " + inputPoints.Length + " points)");
			}
