...
/// <summary>
/// File Dialog Explorer written by Matej Vanco (originally 08/08/2017, Last Update 25/04/2022 [dd/mm/yyyy]).
...
public class FDE_Source : MonoBehaviour 
{
    ...
    private readonly List<string> ProhibitedFiles = new List<string> { "$recycle.bin", "system volume information", "documents and settings", "recovery", "hiberfil", "pagefile" };
    ...
    private IEnumerator A_Internal_RefreshAsyncContent()
    {
        ...
        for (int i = 0; i < Directory.GetDirectories(MainPath).Length; i++)
        {
            ...
            try
            {
                if (ProhibitedFiles.Contains(Path.GetFileName(currentPath).ToLower()))
                    continue;
                try
                {
        ...
        for (int i = 0; i < Directory.GetFiles(MainPath).Length; i++)
        {
            ...
            {
                bool found = false;
                foreach (RegisteredExtensions regExts in registeredExtensions)
                {
    ...
    private bool A_IsImage(string path)
    {
        string ext = Path.GetExtension(path).ToLower();
        return !(ext != ".png" && ext != ".jpg" && ext != ".bmp" && ext != ".tga" && ext != ".gif");
    }
