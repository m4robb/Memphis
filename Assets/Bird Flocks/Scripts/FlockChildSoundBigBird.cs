...
public class FlockChildSoundBigBird:MonoBehaviour{
    ...
    FlockChildBigBird _flockChild;
    AudioSource _audio;
    bool _hasLanded;
    
    public void Start() {
    	_flockChild = GetComponent<FlockChildBigBird>();
    ...
    private void Update()
    {
        ...
        if(AudioGridTick.AudioGridInstance && !AudioTrigger)
        {
            AudioTrigger = true;

            int Multix1 = Random.Range(1, 3);
            int Multix2 = Random.Range(1, 3);
            _audio = GetComponent<AudioSource>();
        InvokeRepeating("PlayRandomSound", (float)AudioGridTick.AudioGridInstance.GetDelayTime() + (float)AudioGridTick.AudioGridInstance.bpm / 60 * Multix1, (float)AudioGridTick.AudioGridInstance.bpm / 60 * 2);
        if (_scareSounds.Length > 0)
            InvokeRepeating("ScareSound", (float)AudioGridTick.AudioGridInstance.GetDelayTime() + (float)AudioGridTick.AudioGridInstance.bpm / 60 * Multix1, (float)AudioGridTick.AudioGridInstance.bpm / 60 *2);
        }
