...
public class FlockControllerBigBird:MonoBehaviour{
    
    public FlockChildBigBird _childPrefab;			// Assign prefab with FlockChild script attached
    ...
    public List<FlockChildBigBird> _roamers;
    ...
    public void AddChild(int amount){
    	...
    	for(int i=0;i<amount;i++){
    		FlockChildBigBird obj = (FlockChildBigBird)Instantiate(_childPrefab);	
    	    obj._spawner = this;
    	    _roamers.Add(obj);
    ...
    public void RemoveChild(int amount){
    	for(int i=0;i<amount;i++){
    		FlockChildBigBird dObj = _roamers[_roamers.Count-1];
    ...
    public void SetFlockRandomPosition() {
        ...
        {
            _posBuffer = WaypointTarget.position;
            return;
        }
