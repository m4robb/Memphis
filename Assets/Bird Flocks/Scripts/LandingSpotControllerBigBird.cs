...
public class LandingSpotControllerBigBird:MonoBehaviour{
    ...
    public FlockControllerBigBird _flock;							// Assign the FlockController to pick birds from
    ...
    public void Start() {
    	...
    	if(_flock == null){
    	 _flock = (FlockControllerBigBird)GameObject.FindObjectOfType(typeof(FlockControllerBigBird));
    ...
    public void ScareAll(float minDelay,float maxDelay) {
    	for(int i=0;  i< _thisT.childCount; i++){
    		if(_thisT.GetChild(i).GetComponent<LandingSpotBigBird>() != null){
                LandingSpotBigBird spot = _thisT.GetChild(i).GetComponent<LandingSpotBigBird>();
