...
public class FlockChild:MonoBehaviour{
    ...
    public void Update() {
        ...
        if(!Anim) Anim = _model.GetComponent<Animator>();
    ...
    public void OnEnable() {
    	if(_instantiated){
    		...
    		if(_landing){
                if (Anim)
                {
                    Anim.Play(_spawner._idleAnimation);
                }
                else
                {
                ...
                if (Anim)
                {
                    Anim.Play(_spawner._flapAnimation);
                }
                else
                {
    ...
    public void Flap(){
    	if(_move){
            if (this._model != null)
            {
                ...
                if (Anim)
                {
                    Anim.CrossFadeInFixedTime(_spawner._flapAnimation, 1f);
                }
                else
                {
    ...
    public void Soar(){
    	if(_move){
            ...
            if (Anim)
            {
                Anim.CrossFadeInFixedTime(_spawner._soarAnimation, 1f);
            }
            else
            {
    ...
    public void animationSpeed(){
        ...
        if (Anim)
        {
            if (!_dived && !_landing)
            {
                Anim.speed = Random.Range(_spawner._minAnimationSpeed, _spawner._maxAnimationSpeed);
            }
            else
            {
                Anim.speed = _spawner._maxAnimationSpeed;
            }
        }
        else
        {
